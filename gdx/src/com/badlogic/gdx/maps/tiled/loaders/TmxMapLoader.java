...
public class TmxMapLoader extends BaseTmxMapLoader<TmxMapLoader.Parameters> {
	...
	public TiledMap load (String fileName, TmxMapLoader.Parameters parameters) {
		try {
			...
			for (FileHandle textureFile : findDependencies(root, tmxFile)) {
				Texture texture = new Texture(textureFile, parameters.generateMipMaps);
				texture.setFilter(parameters.textureMinFilter, parameters.textureMagFilter);
				textures.put(textureFile.path(), texture);
			}
			DirectImageResolver imageResolver = new DirectImageResolver(textures);
			TiledMap map = loadMap(root, tmxFile, imageResolver);
	...
	public void loadAsync (AssetManager manager, String fileName, FileHandle tmxFile, TmxMapLoader.Parameters parameter) {
		...
		try {
			map = loadMap(root, tmxFile, new AssetManagerImageResolver(manager));
		} catch (Exception e) {
			throw new GdxRuntimeException("Couldn't load tilemap '" + fileName + "'", e);
		}
	}
	...
	public Array<AssetDescriptor> getDependencies (String fileName, FileHandle tmxFile, Parameters parameter) {
		...
		try {
			...
			if (parameter != null) {
				texParams.genMipMaps = parameter.generateMipMaps;
				texParams.minFilter = parameter.textureMinFilter;
				texParams.magFilter = parameter.textureMagFilter;
			}
			for (FileHandle image : findDependencies(root, tmxFile)) {
	...
	protected Array<FileHandle> findDependencies(Element root, FileHandle tmxFile) throws IOException {
		Array<FileHandle> images = new Array<FileHandle>();
		for (int index = 0, count = root.getChildCount(); index < count; index++) {
			Element child = root.getChild(index);
			String childName = child.getName();			
			if (childName.equals("tileset")) {
				String source = child.getAttribute("source", null);
				FileHandle image = null;
				if (source != null) {
					FileHandle tsx = getRelativeFileHandle(tmxFile, source);
					child = xml.parse(tsx);
					String imageSource = child.getChildByName("image").getAttribute("source");
					image = getRelativeFileHandle(tsx, imageSource);
				} else {
					image = getRelativeFileHandle(tmxFile, child.getChildByName("image").getAttribute("source"));
				}
				images.add(image);
			} else if (childName.equals("imagelayer")) {
				FileHandle image = getRelativeFileHandle(tmxFile, child.getChildByName("image").getAttribute("source"));
				images.add(image);
			}
		}
		return images;		
	...
	protected TiledMap loadMap (Element root, FileHandle tmxFile, ImageResolver imageResolver) {
		...
		for (int index = 0, count = root.getChildCount(); index < count; index++) {
			...
			if (childName.equals("properties")) {
				loadProperties(map.getProperties(), child);
			} else if (childName.equals("tileset")) {
				loadTileset(map, child, tmxFile, imageResolver);	
			} else if (childName.equals("layer")) {
				loadLayer(map, child);
			} else if (childName.equals("objectgroup")) {
				loadObjectGroup(map, child);
			} else if (childName.equals("imagelayer")) {
				loadImageLayer(map, child);
			}
		}
		return map;
	}
