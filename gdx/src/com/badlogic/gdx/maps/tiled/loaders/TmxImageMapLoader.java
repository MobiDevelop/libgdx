...
public class TmxImageMapLoader extends TmxMapLoader<TiledMap, TmxImageMapLoader.Parameters> {
	...
	public TmxImageMapLoader () {
		super(new InternalFileHandleResolver());
	}

	public TmxImageMapLoader (FileHandleResolver resolver) {
		super(resolver);
	}
	...
	private Array<FileHandle> loadTilesets (Element root, FileHandle mapFile) throws IOException {
		...
		for (Element tileset : root.getChildrenByName("tileset")) {
			...
			if (source != null) {
				FileHandle tsx = mapFile.relative(source);
				tileset = reader.parse(tsx);
				String imageSource = tileset.getChildByName("image").getAttribute("source");
				image = tsx.relative(imageSource);
			} else {
				String imageSource = tileset.getChildByName("image").getAttribute("source");
				image = mapFile.relative(imageSource);
			}
			images.add(image);
		}
		return images;
